---
title: "Power Analysis for Wampus Creek"
author: "Carl Schwarz"
...
# If the Excel file cannot be read, use the csv file
workbookName = file.path('Data','Wampus Creek.xls')
sheetName    = 'F&W'
csvfilename  = file.path("Data","Wampus Creek.csv")

# CHoose one of the following code fragments, depending if you can read the raw Excel file.
...
A preliminary plot of the CPUE over time for each of the species is found in Figure `r prelim.FIGNUM`. Notice that even if the total number of fish captured is very small, 
there may many locations with imputed catches of 0 fish and so the plot may have
many more points than expected.
...
plots.list <- plyr::dlply(power.res, "Species.Code", function(power){
   ...
   power.plot <- ggplot2::ggplot(data=plotdata, aes(x=sites.per.year, 
                                                    y=power.1sided.a, color=PerChangeF))+
