...
#-------------------------------------------------------
# Part 1. Read in the raw data and estimate the variance components.
#         These variance components are then passed to the power program
#         to estimate the power to detect various trends
#

fish <- read.xlsx("FWMIS Loadform BLTR 2016 Nordegg and Tribs.xls", sheetName="Electrofishing",
                 header=TRUE,
                 startRow=3,
                 colIndex=1:49, colClasses='character',
...
# Pull out the species codes
xtabs(~Species.Code, data=fish, exclude=NULL, na.action=na.pass)
fish$Species.Code <- substring(fish$Species.Code, nchar(fish$Species.Code)-3)
xtabs(~Species.Code, data=fish, exclude=NULL, na.action=na.pass)

# convert the oberved count to numeric and remove all NA
xtabs(~Observed.Count, data=fish, exclude=NULL, na.action=na.pass)
# remove any > sign
fish$Observed.Count <- gsub(">","", fish$Observed.Count, fixed=TRUE)
fish$Observed.Count <- as.numeric(fish$Observed.Count)
fish <- fish[ !is.na(fish$Observed.Count),]
xtabs(~Observed.Count, data=fish, exclude=NULL, na.action=na.pass)

# Summarized to the Site, Date level
fish.sum <- plyr::ddply(fish, c("Location..","Start.Date","Time..seconds.","Distance..m.","Species.Code"),
                        plyr::summarize, Observed.Count=sum(Observed.Count, na.rm=TRUE))
# Remove records from non-target species
fish.sum <- fish.sum[ fish.sum$Species.Code %in% target.species,]
dim(fish.sum)

# add 0's for missing records
zero.count <- ldply(target.species, function(Species.Code, site.date){
    site.date$Species.Code   <- Species.Code
    site.date$Observed.Count <- 0
    site.date
}, site.date=site.date)

fish.sum <- rbind(fish.sum, zero.count)
fish.sum <- plyr::ddply(fish.sum, c("Location..","Start.Date","Time..seconds.","Distance..m.","Species.Code"),
                        plyr::summarize, Observed.Count=sum(Observed.Count, na.rm=TRUE))
fish.sum
...
Line 360 missing information such a location #, time (seconds) distance (etc)
Line 1506, 1563, 1650 - Floating awith difference location - should this be included?
Species codes 'Uknown' and "Unknown", "Trout SPP",
Only intrested in "BKRT","BLTR","MNWH" and???
Counts of >10 are converted to 10.
