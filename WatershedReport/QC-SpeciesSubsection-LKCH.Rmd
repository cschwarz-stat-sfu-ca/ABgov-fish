...
```{r LKCHSummary, echo=FALSE, message=FALSE, warning=FALSE }
...
LKCH.tab.fishsummary<-getTABNUM()
LKCH.fig.forklength <-getFIGNUM()

```
The  mean fork length and size range for this species on a yearly basis are summarized in 
Table `r LKCH.tab.fishsummary` 
and plotted in 
Figure `r LKCH.fig.forklength`.

```{r LKCH.fish.summary.tab,  echo=FALSE, results='asis'}
...
pandoc.table(fish.table[,-1],
             caption=paste("Table ", LKCH.tab.fishsummary,". Summary statistics on fork length for ",
                           species.select,
...
```{r LKCH.forklength.fig,  echo=FALSE, message=FALSE, warning=FALSE, fig.height=3, fig.width=5, dpi=300}
...
sumstatplot <- ggplot2::ggplot(data=sumstat.melt, aes(x=Year, y=Length, color=Measure))+
   ggtitle(paste('Figure ', LKCH.fig.forklength,'. Trends in fork length for ', species.select,
...
```{r LKCH.lengthdist, echo=FALSE, message=FALSE, warning=FALSE, fig.height=3, fig.width=5, dpi=300}
...
lengthdist <- ggplot2::ggplot(data=cpue.select, aes(x=Fork.Length..mm., y=..density..))+
   ggtitle(paste("Figure ", fig.LKCH.lengthdist,". Fork length distribution for ", species.select,' on ',
...
```{r LKCH.row.trend.fignum, echo=FALSE}
   # get the figure number for the CPUE plot
   fig.LKCH.rawtrend<-getFIGNUM()
```

A plot of the CPUE over time is shown in 
Figure `r fig.LKCH.rawtrend`.

```{r  LKCH.raw.trend, echo=FALSE, message=FALSE, warning=FALSE, fig.height=3, fig.width=5, dpi=300}
...
raw.trend <- ggplot2::ggplot(data=catch.summary.select, aes(x=Year, y=CPUE.300m))+
   ggtitle(paste('Figure ', fig.LKCH.rawtrend, ". CPUE data for ", species.select, 
...
```{r  LKCH.fit.Bayesian, echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
...
```{r LKCH.Bayes.output.3years, eval=(nyears>=3), results='asis', echo=FALSE}
...
```
```{r LKCH.Bayes.output.2years, eval=(nyears==2), results='asis', echo=FALSE}
...
```
```{r LKCH.Bayes.output.1years, eval=(nyears==1), results='asis', echo=FALSE}
...
```{r LKCH.postplot.fignum, echo=FALSE}
   # get the figure numbers for the posterior plot and the fsi plot
   fig.LKCH.postplot<-getFIGNUM()
   fig.LKCH.fsiplot <-getFIGNUM()
```
...
Figure `r fig.LKCH.postplot` and
Figure `r fig.LKCH.fsiplot`.


```{r  LKCH.postplot, echo=FALSE, message=FALSE, warning=FALSE, fig.height=3, fig.width=5, dpi=300}
fsi.res$postplot+
    ylim(c(0,150))+
   ggtitle(paste('Figure ', fig.LKCH.postplot, ". Estimated MEDIAN trend line for ", species.select,  
                 " at ", catch.summary.select$WatershedName[1], " with FSI categories",sep=""))+
   theme(plot.title = element_text(size = 8))+  # change text size for title
   ylab("CPUE per 300 m")
```
...
```{r  LKCH.fsi.plot, echo=FALSE, message=FALSE, warning=FALSE, fig.height=3, fig.width=5, dpi=300}
...
fsi.res$fsi.plot+
   ggtitle(paste('Figure ', fig.LKCH.fsiplot, ". Posterior probability by FSI category for ", species.select,  
                 " at ", catch.summary.select$WatershedName[1],sep=""))+
   theme(plot.title = element_text(size = 8))+  # change text size for title
