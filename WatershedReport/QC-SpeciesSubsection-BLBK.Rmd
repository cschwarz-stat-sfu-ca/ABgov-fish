...
```{r BLBK.mature.FL, echo=FALSE}
# Extract the species maturity forklength
BLBK.MatureFL <- SpeciesMaturityFL[ SpeciesMaturityFL$Species.Code == species.select, "MatureFL..mm."]
```

The length distribution over all years is shown in 
Figure `r fig.BLBK.lengthdist`.
Black vertical line indicates estimated length at 50\% maturity (`r BLBK.MatureFL` mm Fork Length).
...
lengthdist <- ggplot2::ggplot(data=cpue.select, aes(x=Fork.Length..mm., y=..density..))+
   ...
   geom_density()+
   geom_vline(xintercept=BLBK.MatureFL, color="black",size=2)+
