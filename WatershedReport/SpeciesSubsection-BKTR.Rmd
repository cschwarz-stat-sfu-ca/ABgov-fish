...
```{r BKTRSummary, echo=FALSE, message=FALSE, warning=FALSE }
# Comput the summary statstics for each species by year
fish.select <- fish[ fish$Species.Code == species.select,]

fish.stat <- plyr::ddply(fish.select, c("Species.Code","Year"), plyr::summarize,
...
BKTR.tab.fishsummary<-getTABNUM()
BKTR.fig.forklength <-getFIGNUM()

```
The  mean fork length and size range for this species on a yearly basis are summarized in 
Table `r BKTR.tab.fishsummary` 
and plotted in 
Figure `r BKTR.fig.forklength`.

```{r BKTR.fish.summary.tab,  echo=FALSE, results='asis'}
...
pandoc.table(fish.table[,-1],
             caption=paste("Table ", BKTR.tab.fishsummary,". Summary statistics on fork length for ",
                           species.select,
                           " captured in ",fish$WatershedName[1], sep=""),
             justify='rrrrr',
             split.cells=c(1,1,1,1,1))
```


```{r BKTR.forklength.fig,  echo=FALSE, message=FALSE, warning=FALSE, fig.height=3, fig.width=5, dpi=300}
...
sumstatplot <- ggplot2::ggplot(data=sumstat.melt, aes(x=Year, y=Length, color=Measure))+
   ggtitle(paste('Figure ', BKTR.fig.forklength,'. Trends in fork length for ', species.select,
                 ' on ', fish.select$WatershedName[1],sep=""))+
...
```{r BKTR.lengthdis.fignum, echo=FALSE}
# Get the figure number for the length distribution
fig.BKTR.lengthdist<-getFIGNUM()
```

```{r BKTR.mature.FL, echo=FALSE}
# Extract the species maturity forklength
BKTR.MatureFL <- SpeciesMaturityFL[ SpeciesMaturityFL$Species.Code == species.select, "MatureFL..mm."]
```

The length distribution over all years is shown in 
Figure `r fig.BKTR.lengthdist`.
Black vertical line indicates estimated length at 50\% maturity (`r BKTR.MatureFL` mm Fork Length).

```{r BKTR.lengthdist, echo=FALSE, message=FALSE, warning=FALSE, fig.height=3, fig.width=5, dpi=300}
# Create a histogram of the fork length over all years
lengthdist <- ggplot2::ggplot(data=fish.select, aes(x=Fork.Length..mm., y=..density..))+
   ggtitle(paste("Figure ", fig.BKTR.lengthdist,". Fork length distribution for ", species.select,' on ',
                 fish.select$WatershedName[1],sep=""))+
   theme(plot.title = element_text(size = 8))+  # change text size for title
   geom_histogram(alpha=0.2)+ 
   geom_density()+
   geom_vline(xintercept=BKTR.MatureFL, color="black",size=2)+
   xlab("Fork length (mm)")+ylab("Density")
lengthdist
```

```{r BKTR.row.trend.fignum, echo=FALSE}
   # get the figure number for the CPUE plot
   fig.BKTR.rawtrend<-getFIGNUM()
```

A plot of the CPUE over time is shown in 
Figure `r fig.BKTR.rawtrend`.

```{r  BKTR.raw.trend, echo=FALSE, message=FALSE, warning=FALSE, fig.height=3, fig.width=5, dpi=300}
...
raw.trend <- ggplot2::ggplot(data=catch.summary.select, aes(x=Year, y=CPUE.300m))+
   ggtitle(paste('Figure ', fig.BKTR.rawtrend, ". CPUE data for ", species.select, 
...
```{r  BKTR.fit.Bayesian, echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
...
```{r BKTR.Bayes.output.3years, eval=(nyears>=3), results='asis', echo=FALSE}
...
```
```{r BKTR.Bayes.output.2years, eval=(nyears==2), results='asis', echo=FALSE}
...
```
```{r BKTR.Bayes.output.1years, eval=(nyears==1), results='asis', echo=FALSE}
...
```{r BKTR.postplot.fignum, echo=FALSE}
   # get the figure numbers for the posterior plot and the fsi plot
   fig.BKTR.postplot<-getFIGNUM()
   fig.BKTR.fsiplot <-getFIGNUM()
```
...
Figure `r fig.BKTR.postplot` and
Figure `r fig.BKTR.fsiplot`.


```{r  BKTR.postplot, echo=FALSE, message=FALSE, warning=FALSE, fig.height=3, fig.width=5, dpi=300}
fsi.res$postplot+
    ylim(c(0,150))+
   ggtitle(paste('Figure ', fig.BKTR.postplot, ". Estimated MEDIAN trend line for ", species.select,  
                 " at ", catch.summary.select$WatershedName[1], " with FSI categories",sep=""))+
   theme(plot.title = element_text(size = 8))+  # change text size for title
   ylab("CPUE per 300 m")
```
...
```{r  BKTR.fsi.plot, echo=FALSE, message=FALSE, warning=FALSE, fig.height=3, fig.width=5, dpi=300}
...
fsi.res$fsi.plot+
   ggtitle(paste('Figure ', fig.BKTR.fsiplot, ". Posterior probability by FSI category for ", species.select,  
                 " at ", catch.summary.select$WatershedName[1],sep=""))+
   theme(plot.title = element_text(size = 8))+  # change text size for title
