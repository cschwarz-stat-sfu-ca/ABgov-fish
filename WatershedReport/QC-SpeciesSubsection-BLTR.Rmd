...
```{r BLTRSummary, echo=FALSE, message=FALSE, warning=FALSE }
...
BLTR.tab.fishsummary<-getTABNUM()
BLTR.fig.forklength <-getFIGNUM()

```
The  mean fork length and size range for this species on a yearly basis are summarized in 
Table `r BLTR.tab.fishsummary` 
and plotted in 
Figure `r BLTR.fig.forklength`.

```{r BLTR.fish.summary.tab,  echo=FALSE, results='asis'}
...
pandoc.table(fish.table[,-1],
             caption=paste("Table ", BLTR.tab.fishsummary,". Summary statistics on fork length for ",
                           species.select,
...
```{r BLTR.forklength.fig,  echo=FALSE, message=FALSE, warning=FALSE, fig.height=3, fig.width=5, dpi=300}
...
sumstatplot <- ggplot2::ggplot(data=sumstat.melt, aes(x=Year, y=Length, color=Measure))+
   ggtitle(paste('Figure ', BLTR.fig.forklength,'. Trends in fork length for ', species.select,
...
```{r BLTR.lengthdis.fignum, echo=FALSE}
# Get the figure number for the length distribution
fig.BLTR.lengthdist<-getFIGNUM()
```

```{r BLTR.mature.FL, echo=FALSE}
# Extract the species maturity forklength
BLTR.MatureFL <- SpeciesMaturityFL[ SpeciesMaturityFL$Species.Code == species.select, "MatureFL..mm."]
```

The length distribution over all years is shown in 
Figure `r fig.BLTR.lengthdist`.
Black vertical line indicates estimated length at 50\% maturity (`r BLTR.MatureFL` mm Fork Length).

```{r BLTR.lengthdist, echo=FALSE, message=FALSE, warning=FALSE, fig.height=3, fig.width=5, dpi=300}
...
lengthdist <- ggplot2::ggplot(data=cpue.select, aes(x=Fork.Length..mm., y=..density..))+
   ggtitle(paste("Figure ", fig.BLTR.lengthdist,". Fork length distribution for ", species.select,' on ',
                 cpue.select$WatershedName[1],sep=""))+
   theme(plot.title = element_text(size = 8))+  # change text size for title
   geom_histogram(alpha=0.2)+ 
   geom_density()+
   geom_vline(xintercept=BLTR.MatureFL, color="black",size=2)+
   xlab("Fork length (mm)")+ylab("Density")
lengthdist
```

```{r BLTR.row.trend.fignum, echo=FALSE}
   # get the figure number for the CPUE plot
   fig.BLTR.rawtrend<-getFIGNUM()
```

A plot of the CPUE over time is shown in 
Figure `r fig.BLTR.rawtrend`.

```{r  BLTR.raw.trend, echo=FALSE, message=FALSE, warning=FALSE, fig.height=3, fig.width=5, dpi=300}
...
raw.trend <- ggplot2::ggplot(data=catch.summary.select, aes(x=Year, y=CPUE.300m))+
   ggtitle(paste('Figure ', fig.BLTR.rawtrend, ". CPUE data for ", species.select, 
...
```{r  BLTR.fit.Bayesian, echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
...
```{r BLTR.Bayes.output.3years, eval=(nyears>=3), results='asis', echo=FALSE}
...
```
```{r BLTR.Bayes.output.2years, eval=(nyears==2), results='asis', echo=FALSE}
...
```
```{r BLTR.Bayes.output.1years, eval=(nyears==1), results='asis', echo=FALSE}
...
```{r BLTR.postplot.fignum, echo=FALSE}
   # get the figure numbers for the posterior plot and the fsi plot
   fig.BLTR.postplot<-getFIGNUM()
   fig.BLTR.fsiplot <-getFIGNUM()
```
...
Figure `r fig.BLTR.postplot` and
Figure `r fig.BLTR.fsiplot`.


```{r  BLTR.postplot, echo=FALSE, message=FALSE, warning=FALSE, fig.height=3, fig.width=5, dpi=300}
fsi.res$postplot+
    ylim(c(0,150))+
   ggtitle(paste('Figure ', fig.BLTR.postplot, ". Estimated MEDIAN trend line for ", species.select,  
                 " at ", catch.summary.select$WatershedName[1], " with FSI categories",sep=""))+
   theme(plot.title = element_text(size = 8))+  # change text size for title
   ylab("CPUE per 300 m")
```
...
```{r  BLTR.fsi.plot, echo=FALSE, message=FALSE, warning=FALSE, fig.height=3, fig.width=5, dpi=300}
...
fsi.res$fsi.plot+
   ggtitle(paste('Figure ', fig.BLTR.fsiplot, ". Posterior probability by FSI category for ", species.select,  
                 " at ", catch.summary.select$WatershedName[1],sep=""))+
   theme(plot.title = element_text(size = 8))+  # change text size for title
